...
def visualization(message, bot) -> None:
    ...
    if count >= 5:
        if len(get_multiple(village)) <= 5:
            ...
            create_buttons_multiple_locality(bot=bot, message=message, counties=get_multiple(village))
    else:
    ...
    for i in churches:
        ...
        if len(messanges) > 4096:
            for x in range(0, len(messanges), 4096):
                bot.send_message(message.chat.id, messanges[x:x + 4096], parse_mode='Markdown')
        else:
